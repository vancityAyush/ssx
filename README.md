...
`sshx` is a TypeScript-first CLI for generating and configuring SSH keys for GitHub, GitLab, Bitbucket, and Azure DevOps. The primary implementation now lives in TypeScript, while the original shell scripts remain available as fallbacks.
...
Those remain useful fallbacks, but the main project language and CLI implementation now live in [`src/cli.ts`](/Users/vancityayush/Development/ssh_script/src/cli.ts).
